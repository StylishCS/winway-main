...
async function getCartItems(studentId){
    const query = util.promisify(connection.query).bind(connection)
    const cart = await query("SELECT * FROM cart WHERE studentId = ?" , [studentId])
    return cart;
}
...
module.exports ={createCart ,removeCart, checkIfExists, getCartItems}
